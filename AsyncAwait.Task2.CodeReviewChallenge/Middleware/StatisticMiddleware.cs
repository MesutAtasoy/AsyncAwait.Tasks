...
public class StatisticMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        //var staticRegTask = Task.Run(() => _statisticService.RegisterVisitAsync(path).ConfigureAwait(false).GetAwaiter().OnCompleted(UpdateHeaders));
        //Console.WriteLine(staticRegTask.Status); // just for debugging purposes

        //void UpdateHeaders()
        //{
        //    context.Response.Headers.Add(CustomHttpHeaders.TotalPageVisits, _statisticService.GetVisitsCountAsync(path).GetAwaiter().GetResult().ToString());
        //}
